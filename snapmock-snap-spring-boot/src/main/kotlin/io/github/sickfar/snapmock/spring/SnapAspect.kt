...
/**
 * This aspect processes beans and bean methods annotated [io.github.sickfar.snapmock.core.Snap]
 *
 * @since 1.0.0
...
open class SnapAspect(
    ...
    @Pointcut("@annotation(io.github.sickfar.snapmock.core.Snap)")
    open fun onMethodAnnotatedSnap() {
        // pointcut definition
    }

    @Pointcut("execution(public * *(..)) && within(@io.github.sickfar.snapmock.core.Snap *)")
    ...
    open fun aroundMethod(joinPoint: ProceedingJoinPoint): Any? {
        ...
        try {
            storage.start()
            val result: Any? = signature.method.invoke(newTarget, *joinPoint.args).also { res: Any? ->
                postprocessResult(signature.returnType, res)
            }
    ...
    private fun buildDependencySpy(
        parameterType: Class<*>,
        dependency: Any?,
        isFactoryByFieldOrCtorArg: Boolean
    ): Any? {
        if (dependency == null) {
            return null
        }
